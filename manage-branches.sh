...
function deploy {
    ...
    if [[ "$(yq '.resources[]' "$STAGING_KUSTOMIZATION")" =~ "settings-ui-$CI_COMMIT_BRANCH" ]]; then
        err "settings-ui-$CI_COMMIT_BRANCH is already configured"
    fi
    yq -i '.resources += ["settings-ui-'$CI_COMMIT_BRANCH'"]' "$STAGING_KUSTOMIZATION"
...
function destroy {
    ...
    rm -r "$DESTROY_TARGET"
    yq -i 'del(.resources[] | select(. == "settings-ui-'$CI_COMMIT_BRANCH'"))' "$STAGING_KUSTOMIZATION"
    cd "$DEPLOYMENT_DIR"
    git add .
